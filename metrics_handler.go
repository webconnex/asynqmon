...
func newGetMetricsHandlerFunc(client *http.Client, prometheusAddr string) http.HandlerFunc {
	...
	const (
		...
		promQLProcessedTasks = "rate(asynq_tasks_processed{QUEUE_FILTER}[5m])"
		promQLFailedTasks    = "rate(asynq_tasks_failed{QUEUE_FILTER}[5m])"
		promQLErrorRate      = "rate(asynq_tasks_failed{QUEUE_FILTER}[5m]) / rate(asynq_tasks_processed{QUEUE_FILTER}[5m])"
